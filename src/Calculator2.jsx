...
import OperationButton from "./Calculator2"; 
import "./styles.css"; 
...
state.previousOperand == null){ 
    ...
    case ACTIONS.EQUAL: 
      if (state.operation == null || state.currentOperand 
== null || state.previousOperand == null){ 
        return state 
      } 
      return{ 
        ...state, 
...
operation},dispatch] = useReducer(reducer,{}) 
...
operand">{formatOperand(previousOperand)} 
{operation}</div> 
        <div className="current
operand">{formatOperand(currentOperand)}</div> 
      </div> 
...
/> 
      ...
      <button className="span-two" onClick={()=> 
dispatch({type:ACTIONS.EQUAL})}>=</button> 
    </div> 
  ) 
} 
export default App; 